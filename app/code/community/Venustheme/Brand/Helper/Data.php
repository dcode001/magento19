...
-------------------------------------------------------------------------*/
class Venustheme_Brand_Helper_Data extends Mage_Core_Helper_Abstract {
	...
	function get( $attributes = NULL)
	{
		...
		{	    	
			$tags = array('venustheme_brand', 'effect_setting', 'file_source_setting','image_source_setting', 'main_brand');
			foreach($tags as $tag){
				if(Mage::getStoreConfig("venustheme_brand/$tag/$var")!=""){
					$data[$var] =  Mage::getStoreConfig("venustheme_brand/$tag/$var");
				}
			}
        ...
        return Mage::getSingleton('venustheme_brand/config')->getBaseMediaUrl() . $url;
    }
	public function resizeImage( $image,$type="l", $width, $height ){
		 
		$imageProcessor =  Mage::helper('venustheme_brand/vesimage');
	...
	public function getCategoriesList( $default=true ){
		...
		$collection =  Mage::getModel('venustheme_brand/category')->getCollection();
	...
	public function getTagUrl($tag){
		return Mage::getBaseUrl().Mage::getStoreConfig('venustheme_brand/general_setting/route')."/tag/".$tag.".html";
	}
	...
	public function getBrandById( $id ){
		return Mage::getModel("venustheme_brand/brand")->load( $id ) ;
	}
}
?>