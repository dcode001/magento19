...
-------------------------------------------------------------------------*/
class Venustheme_Brand_Model_Observer  extends Varien_Object
{
	...
	public function initControllerRouters( $observer ){	
        ...
        if (!$condition->getContinue())
            return false;
		$route = trim( Mage::getStoreConfig('venustheme_brand/general_setting/route') );
		if($identifier) {
	...
	//	$menu_name = $controller_name."_".Mage::app()->getRequest()->getControllerName();
		$helper =  Mage::helper('venustheme_brand/data');
   ...
   public function brandScrollModule( $menu_name, $helper ){
		...
		if($helper->checkMenuItem( $menu_name, $this->getModuleConfig("scroll_menuassignment") )){
			...
			$block =  $layout->createBlock( 'venustheme_brand/scroll' );
    ...
    public function brandNavModule( $menu_name, $helper ){
		...
		if($helper->checkMenuItem( $menu_name, $this->getModuleConfig("brandnav_menuassignment") )){
			...
			$block =  $layout->createBlock( 'venustheme_brand/brandnav' );
   ...
   function _loadMedia( $config = array()){
		...
		$mediaHelper =  Mage::helper('venustheme_brand/media');
		if( $config['enable_jquery'] ){
		 	$mediaHelper->addMediaFile("js","venustheme_brand/jquery.js");
		}
		$mediaHelper->addMediaFile("js","venustheme_brand/script.js");
		$mediaHelper->addMediaFile("skin_css", "venustheme_brand/style.css" );
	}
}
