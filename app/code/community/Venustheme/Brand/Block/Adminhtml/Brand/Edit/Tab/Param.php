...
class Venustheme_Brand_Block_Adminhtml_Category_Edit_Tab_Param extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        ...
        $fieldset = $form->addFieldset('category_params', array('legend'=>Mage::helper('venustheme_brand')->__('Parameter')));
		
		$fieldset->addField('template', 'select', array(
			'label'     => Mage::helper('venustheme_brand')->__('Template'),
			'name'      => 'param[template]',
			'values'    => array( 0=> $this->__("No"), 1=> $this->__("Yes") )
		));
		
		$fieldset->addField('show_childrent', 'select', array(
			'label'     => Mage::helper('venustheme_brand')->__('Show Childrent'),
	  ...
	  public function getParentToOptionArray() {
		$catCollection = Mage::getModel('venustheme_brand/category')
					->getCollection();
