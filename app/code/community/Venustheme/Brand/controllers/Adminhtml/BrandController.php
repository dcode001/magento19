...
-------------------------------------------------------------------------*/
class Venustheme_Brand_Adminhtml_BrandController extends Mage_Adminhtml_Controller_Action {
    protected function _initAction() {
        $this->loadLayout()
                ->_setActiveMenu('venustheme_brand/brand');
    ...
    public function indexAction() {
        ...
        $this->_initAction();
        $this->_addContent($this->getLayout()->createBlock('venustheme_brand/adminhtml_brand') );
        $this->renderLayout();
	...
	public function editAction(){
        ...
        $_model  = Mage::getModel('venustheme_brand/brand')->load( $id );
		...
		$this->loadLayout();
	    $this->_setActiveMenu('venustheme_brand/brand');
        ...
        $this->_addContent($this->getLayout()->createBlock('venustheme_brand/adminhtml_brand_edit'))
                ->_addLeft($this->getLayout()->createBlock('venustheme_brand/adminhtml_brand_edit_tabs'));
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {	    
			$model = Mage::getModel('venustheme_brand/brand');
			if($data['identifier'] == '' || !isset($data['identifier']))
			$data['identifier'] = Mage::helper('venustheme_brand')->formatUrlKey($data['title']);
			...
			if(isset($_FILES['file']['name']) && $_FILES['file']['name'] != '') {					
				...
				foreach( $sizes as $key => $size ){
					$c = Mage::getStoreConfig( 'venustheme_brand/general_setting/'.$key );
					$tmp = explode( "x", $c );
					if( count($tmp) > 0 && (int)$tmp[0] ){
						Mage::helper('venustheme_brand')->resizeImage( $data['file'], $size, (int)$tmp[0], (int)$tmp[1] );
					}
				}		
			}else{
			...
			try {
				$model->save();
				$resroute = Mage::getStoreConfig('venustheme_brand/general_setting/route');
				$extension = ".html";
				//Save to Url Rewite
				...
				Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('venustheme_brand')->__('Brand was successfully saved'));
		...
		Mage::getSingleton('adminhtml/session')->addError(Mage::helper('venustheme_brand')->__('Unable to find cat to save'));
		$this->_redirect('*/*/');
    }
	...
	public function imageAction() {
        ...
        try {
            ...
            $result = $uploader->save(
                    Mage::getSingleton('venustheme_brand/config')->getBaseMediaPath()
            );

            $result['url'] = Mage::getSingleton('venustheme_brand/config')->getMediaUrl($result['file']);
            $result['cookie'] = array(
                    'name'     => session_name(),
	 ...
	 public function deleteAction() {
		...
		if( $this->getRequest()->getParam('id') > 0 ) {
			try {
				...
				Mage::getModel('core/url_rewrite')->loadByIdPath('venustheme_brand/brand/'.$model->getId())->delete();
				...
				if( $post->getFile() ){
					...
					foreach( $sizes as $key => $size ){
						...
						if( count($tmp) > 0 && (int)$tmp[0] ){
							...
							Mage::helper('venustheme_brand')->resizeImage( $post->getFile(), $size, (int)$tmp[0], (int)$tmp[1] );
						}
					}	
				}
			}
        ...
        if(!is_array($IDList)) {
            ...
            try {
                foreach ($IDList as $itemId) {
                    $_model = Mage::getSingleton('venustheme_brand/brand')
	...
	public function massDeleteAction() {
        ...
        if(!is_array($IDList)) {
            ...
            try {
                foreach ($IDList as $itemId) {
                    $_model = Mage::getModel('venustheme_brand/brand')
