...
class Venustheme_TabsHome_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function get($attributes = NULL) {
        ...
        foreach ($arrayParams as $var) {
            $tags = array('venustheme_tabshome', 'main_slider'); //group config
            foreach ($tags as $tag) {
                if (Mage::getStoreConfig("venustheme_tabshome_info/$tag/$var") != "") {
                    $data[$var] = Mage::getStoreConfig("venustheme_tabshome_info/$tag/$var"); //item config
                }
            }
    ...
    public function getImageUrl($url = null) {
        return Mage::getSingleton('venustheme_tabshome/config')->getBaseMediaUrl() . $url;
    }
