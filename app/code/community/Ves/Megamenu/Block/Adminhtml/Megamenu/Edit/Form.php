...
//	 echo '<pre>'.print_r($this->articleTypes,1); die;
    ...
    public function getArticleHtml($type, $disabled) {
		...
		if($type) {
			if($this->getMegamenu()->getId()) {
				$html .= $this->nodeToSelectHtml($type, $this->getMegamenu()->getArticle(), $disabled);
			} else {
    ...
    public function getStoreConfigurationUrl()
    {
	 ...
	 public function nodeToSelectHtml($name, $select = 0, $disabled = false, $multiple = false) {
        ...
        $html .= '<select name="megamenu[article]" id="megamenu[article]" class="input-text required-entry"'.$multiple_html;
        if($disabled)
