...
class Ves_Tempcp_Block_System_Config_Form_Field_Notification extends Mage_Adminhtml_Block_System_Config_Form_Field {
    ...
    protected function _getElementHtml(Varien_Data_Form_Element_Abstract $element) {
        ...
        $jspath = $url . 'ves_tempcp/form/script.js';
        $csspath = $url . 'ves_tempcp/form/style.css';
        $output = '<link rel="stylesheet" type="text/css" href="' . $csspath . '" />';

        $output .= '<script type="text/javascript" src="' . $url .'ves_tempcp/jquery.js'. '"></script>';
        $output .= '<script type="text/javascript" src="' . $url ."ves_tempcp/jquery.easing.js". '"></script>';
